...
fn main() -> iced::Result {
    ...
    let fonts = vec![
        include_bytes!("../assets/LXGWWenKaiGBScreenR.ttf").into(),
    ];
...
impl Application for SilkPlayer {
    ...
    fn new(_flags: Self::Flags) -> (Self, Command<Message>) {
        ...
        util::check_dir_and_create(&format!("{current_dir}/cache"));
        util::check_dir_and_create(&format!("{current_dir}/data"));
        util::check_dir_and_create(&format!("{current_dir}/log"));
...
impl SilkPlayer {
    ...
    fn init(&mut self) {
        self.init_list();
        let volume = self.setting.volume;
        self.audio.set_volume(volume);

        self.play_list.size = 50;

        PlayStatus::load().init(self);
    ...
    fn update_time(&mut self, _msg_time: Instant) -> Command<Message> {
        ...
        for show_lyric in &self.current_song.lyric {
            ...
            if show_lyric.beg - lyric_ahead <= self.audio.position() && self.audio.position() < show_lyric.end - lyric_ahead {
    ...
    /// 缓存专辑封面
    fn init_album_img(&self, vec: Vec<MusicInfo>) {
